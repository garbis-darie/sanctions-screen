 ...
 * Uses Levenshtein distance + normalization for robust matching
...
/**
 * Token overlap ratio between normalized strings.
 */
...
function explainMatch(normQuery, normCandidate) {
  const editDistance = levenshtein(normQuery, normCandidate);
  const score = similarity(normQuery, normCandidate);
  const overlap = tokenOverlap(normQuery, normCandidate);
  return {
    normalized_query: normQuery,
    normalized_candidate: normCandidate,
    edit_distance: editDistance,
    token_overlap_ratio: Math.round(overlap * 1000) / 1000,
...
export function screen(query, options = {}) {
  ...
  for (const entry of list) {
    ...
    const nameScore = similarity(normQuery, normName);
    ...
    const aliasScores = (entry.aliases || []).map((alias) => {
      ...
      return {
        alias,
        normalized,
        score: similarity(normQuery, normalized)
      };
    });
    const bestAlias = aliasScores.sort((a, b) => b.score - a.score)[0] ?? null;
    const bestAliasScore = bestAlias ? bestAlias.score : 0;
    const bestScore      = Math.max(nameScore, bestAliasScore);
    const matchedOn      = bestAliasScore > nameScore ? 'alias' : 'primary_name';

    if (bestScore >= threshold) {
      const candidateName = matchedOn === "alias" ? bestAlias.alias : entry.name;
      const candidateNorm = matchedOn === "alias" ? bestAlias.normalized : normName;
      matches.push({
        entry_id:    entry.id,
        name:        entry.name,
        matched_value: candidateName,
        ...
        ...(include_explainability
          ? { explainability: explainMatch(normQuery, candidateNorm) }
          : {})
      });
    }
  }
